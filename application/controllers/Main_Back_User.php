...
class Main_Back_User extends CI_Controller
{
	...
	{
		//config
		$config['base_url'] = 'http://localhost/BISABISABISA/Main_Back_User/semua_kos/';
		$config['total_rows'] = 1000;
		// $this->User_model->countAllkos()
		$config['per_page'] = 3;
		...
		$data['sql'] = $this->User_model->info_beberapa_kos($config['per_page'], 0);
		$sudah_login = $this->session->userdata('sudah_login');
		$data['id_role'] = $this->session->userdata('id_role');
		$data['jk'] = $this->session->userdata('jk');
		$data['username'] = $this->session->userdata('username');
